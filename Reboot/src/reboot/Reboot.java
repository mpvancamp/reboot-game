...
public class Reboot extends JFrame {
    ...
    public static int MARGIN = 35; // Room at top for status bar.
    ...
    private Waypoint[] waypoints; // Waypoints where NPC's are to move
    ...
    /**
     * Create waypoints which the NPC's will use to perform motion actions.
     */
    public void createWaypoints() {
        waypoints = new Waypoint[10];
        waypoints[0] = new Waypoint(WIDTH - 250, HEIGHT/2 + 220, 5, 5); //Slash bottom left waypoint
        waypoints[1] = new Waypoint(WIDTH - 250, HEIGHT/2 + 100, 5, 5); // Slash top left waypoint
        waypoints[2] = new Waypoint(WIDTH - 200, HEIGHT/2 + 120, 5, 5); // Slash top right waypoint
        waypoints[3] = new Waypoint(WIDTH - 200, HEIGHT/2 + 200, 5, 5); // Slash bottom right waypoint
                                            // Make slash wander between waypoint[0-3] randomly at initiation
        
        waypoints[4] = new Waypoint(WIDTH/2 + 70, 45, 9, 255); //Hack's furthest left wall (Width/2 + 70, 45)
        
        // The following waypoints are not allocated as of yet
        waypoints[5] = new Waypoint(WIDTH - 150, HEIGHT/2 - 150, 2, 2);
        waypoints[6] = new Waypoint(WIDTH - 150, HEIGHT/2 - 150, 2, 2);
        waypoints[7] = new Waypoint(WIDTH - 150, HEIGHT/2 - 150, 2, 2);
        waypoints[8] = new Waypoint(WIDTH - 250, HEIGHT/2 + 100, 5,5);
        waypoints[9] = new Waypoint(WIDTH - 250, HEIGHT/2 + 220, 5, 5);
    ...
    public void moveAll() {
        ...
        slash.move(dot, bob, hack, wires, walls, waypoints);
    }
    ...
    public void checkHits() {
        ...
        //if (slash.isAlive() && checkWaypoints(slash)) {
